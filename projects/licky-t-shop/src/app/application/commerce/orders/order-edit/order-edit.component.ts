...
export class OrderEditComponent extends LickAppPageComponent implements OnInit, OnDestroy, LickAppBehavior {
  ...
  onUpdate(): void {
    ...
    const redirectPath = '/application/stores/' + this.store_id + '/orders/' + this.order.id;
    this.uploadSingle();
  ...
  setBreadCrumb(): void {
    this.crumbs = [
      ...
      { name: "orders", link: "/application/stores/" + this.store_id + "/orders", active: false },
      { name: "new", link: "/application/stores/" + this.store_id + "/orders/new", active: true },
    ]
  }
